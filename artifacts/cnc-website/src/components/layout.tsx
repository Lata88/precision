...
export function Layout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-screen flex flex-col relative">
      ...
      <nav className={cn(
        "sticky top-0 z-50 bg-gradient-to-r from-white via-white to-white/95 backdrop-blur-sm transition-all duration-300 border-b border-gray-100/50",
          ...
          <div className="flex justify-between items-center h-[80px]">
            ...
            <Link href="/" className="flex items-center gap-3 group cursor-pointer shrink-0">
              <img 
                ...
                className={cn(
                  "w-35 h-20 object-contain transition-all duration-300",
                  isLogoAnimated ? "logo-load-animation logo-glow" : ""
                )} 
              />
              <div className={cn(isLogoAnimated ? "logo-text-animation" : "")}>
                <span className="font-display font-bold text-xl tracking-wide text-navy block">
                  PRECISION<span className="text-primary"> MACHINE</span> <span className="text-primary">TOOLS</span> LLC
                </span>
                <span className="text-[10px] font-bold uppercase tracking-widest">Advanced Machining Solutions</span>
              </div>
            </Link>
              ...
              >
                <button
                  ...
                  className={cn(
                    ...
                    location === "/machines" || location === "/tools" || location === "/services"
                      ? "text-primary bg-primary/5 after:absolute after:bottom-0 after:left-2 after:right-2 after:h-0.5 after:bg-primary shadow-sm"
                      : "text-navy hover:text-primary hover:bg-gray-50/50 hover:shadow-sm"
                  )}
                >
        ...
        {/* Mobile Menu */}
        {isMobileMenuOpen && (
          <div className="lg:hidden border-t border-gray-100 bg-white/95 backdrop-blur-sm animate-in slide-in-from-top-2 duration-200">
                    ...
                    {productsDropdown.map((item) => (
                      ...
                      >
                        <div className={cn(
                          ...
                          {item.label === "Machine Tools" && <span>⚙️</span>}
                          {item.label === "Services" && <span>🛠️</span>}
                        </div>
                        <div className="flex-1">
                          <div className="font-medium text-sm">{item.label}</div>
                          <div className="text-xs opacity-75">
                            {item.label === "CNC Machines" && "Advanced CNC machinery"}
                            {item.label === "Machine Tools" && "Precision cutting tools"}
                            {item.label === "Services" && "Expert support & maintenance"}
                          </div>
                        </div>
                      </Link>
                    ))}
            ...
            <div>
              <Link href="/" className="flex items-center gap-3 mb-5">
                ...
                <img 
                  ...
                  alt="Precision Machine Tools LLC" 
                  className={cn(
                    "w-25 h-16 object-contain transition-all duration-300",
